...
```
# Wait briefly for the data cache to be warm (avoids race with /api/data)
time.sleep(1)
try:
    data = get_data()
except Exception as e:
    raise RuntimeError(f"Could not load FRED data for commentary: {e}")
curve = data["curve"]
bei   = data["bei"]
...
prompt = (
    "You are a concise macro analyst. Today is April 2026. "
    "The data below is current and accurate — do not question the dates. "
    "Based on the following recent FRED data, write exactly 3 sentences "
    "summarising current US macro conditions and what they imply for markets. "
    "Be direct and specific — no preamble, no sign-off.\n\n"
...
<script>
// ── ETF regime map (70 ETFs mapped to four macro regimes) ───────────────────────
// Note: KSPY/MG/iJJ/iNDA treated as SPY/MOAT/IJH/INDA equivalents
const ETF_MAP = {
  goldilocks: {
    label: 'Goldilocks',
    long:  ['QQQ','IWF','SMH','AIQ','CIBR','XLK','XLY','XLC','IWM','IJH','HYG','CWB',
            'SPMO','KWEB','FXI','CHIQ','EEM','SPY','KSPY','XRT','XHB','IBIT','PAVE','ITA',
            'XLI','XTN','FEZ','EWJ','EWU','SMIN','VNM','KMOAT','DXJ','XHE','EMB'],
    short: ['GLD','SLV','USO','DBC','DBA','CPER','TLT','SHY','XLU','XLP','SPLV','SDY',
            'MUB','UUP','FXY','EMX'],
    desc:  'Risk-on equities lead. Tech, growth, small-cap, EM and high-yield credit outperform. Real assets, long-duration bonds and defensives lag.'
  },
  boom: {
    label: 'Inflationary boom',
    long:  ['XLE','XLB','XLI','XLF','GLD','SLV','CPER','DBC','DBA','USO','XTN','PAVE',
            'KRE','KIE','ITA','EEM','EWW','EZA','DXJ','IWD','IWM','KSPY','CEW','EMX','VNM',
            'GRID','IJH','UUP','EWJ','FEZ'],
    short: ['TLT','IEF','IEI','QQQ','XLK','IWF','SMH','XLRE','XLU','MUB','FXY','FXE',
            'IBIT','KWEB'],
    desc:  'Cyclicals, commodities and financials lead. Energy, materials, EM and real assets outperform. Long-duration bonds and rate-sensitive growth lag.'
  },
  stagflation: {
    label: 'Stagflation',
    long:  ['GLD','SLV','USO','DBC','DBA','CPER','XLU','XLP','XLV','SPLV','SDY','MUB',
            'SHY','IEI','UUP','XLE','EMX','XHE'],
    short: ['QQQ','IWF','IWM','SPY','KSPY','XLY','XLC','HYG','CWB','EEM','KWEB','FXI','CHIQ',
            'XHB','XRT','SMH','XLRE','TLT','EMB','IBIT','PAVE','XTN','KRE'],
    desc:  'Hardest regime for equities and credit. Real assets and defensives provide shelter. Growth, EM equities and credit under the most pressure.'
  },
  bust: {
    label: 'Bust / deflation',
    long:  ['TLT','IEF','IEI','SHY','GLD','XLU','XLP','XLV','SPLV','SDY','MUB','FXY',
            'EMB','XHE','KMOAT','IWD'],
    short: ['XLE','XLB','XLI','USO','DBC','DBA','HYG','CWB','IWM','KSPY','EEM','XLY','XRT',
            'KRE','XHB','IBIT','SMH','CPER','CEW','VNM','EWW','EZA','KWEB','CHIQ'],
    desc:  'Flight to safety. Long-duration Treasuries and defensive equities outperform. Risk assets, commodities, EM and credit under sustained pressure.'
  }
};
...
friendly = “Anthropic API is overloaded — click Retry in a moment.”
elif “FRED” in msg or “Could not load” in msg:
friendly = “Market data temporarily unavailable — try refreshing the page.”
else:
friendly = “Commentary unavailable right now. Try refreshing in a moment.”
return jsonify({“commentary”: None, “error”: friendly}), 500
except Exception as e:
return jsonify({“commentary”: None,
“error”: “Commentary unavailable right now. Try refreshing in a moment.”}), 500
